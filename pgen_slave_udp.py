...
name = "{}_udp_port_{}_{}_{}.pcap".format(protocol_name, port, sys.argv[3], time.time_ns())
proc = subprocess.Popen(["tcpdump","-i","{}".format(sys.argv[3]),"host","{}".format(host), "-w", "{}".format(name)], shell=False)
time.sleep(0.4)
...
while True:    
    ...
    while True:
        s.sendto(bytes.fromhex(input_data.upper()), (host, port))
        time.sleep(0.4)
        aa = s.recvfrom(65535)
